...
export default async function AssessorParticipantPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: app } = await supabase.from("applications").select("id,status,participant:participants(participant_no,full_name)").eq("id", id).single();
  if (!app) notFound();
  ...
  return <div className="space-y-5"><section className="rpl-card p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-xs font-black text-[var(--rpl-green-800)]">{(app.participant as any)?.participant_no}</div><h1 className="mt-1 text-2xl font-black text-[var(--rpl-green-950)]">{(app.participant as any)?.full_name}</h1><p className="mt-1 text-xs text-[var(--muted)]">Posisi Anda: {assignment.assessor1_id === assessor.id ? "Asesor 1" : "Asesor 2"}</p></div><StatusBadge status={app.status} /></div></section><ScoringForm applicationId={id} status={app.status} claims={rows as any[]} /></div>;
}
