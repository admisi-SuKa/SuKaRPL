...
export default async function ProdiDashboard() {
  ...
  let claims: any[] = [], evidences: any[] = [], assignments: any[] = [];
  if (appIds.length) {
    ...
    claims = c.data || []; evidences = e.data || []; assignments = a.data || [];
  }
      ...
      <section className="rpl-card overflow-hidden">
        ...
        <div className="grid gap-3 p-4 md:hidden">
          {rows.map((row: any) => (
            <Link key={row.id} href={`/prodi/peserta/${row.id}`} className="rounded-2xl border border-[var(--line)] bg-[#fbfdfc] p-4 active:bg-[#f0f8f5]">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-white p-2"><div className="font-black">{row.claimCount}</div><div className="text-[10px] text-[var(--muted)]">Mata Kuliah</div></div><div className="rounded-xl bg-white p-2"><div className="font-black">{row.evidenceCount}</div><div className="text-[10px] text-[var(--muted)]">Bukti</div></div><div className="rounded-xl bg-white p-2"><div className="font-black">{row.assigned ? "2" : "-"}</div><div className="text-[10px] text-[var(--muted)]">Asesor</div></div></div>
            </Link>
          ))}
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            ...
            <tbody className="divide-y divide-[var(--line)]">{rows.map((row: any) => <tr key={row.id} className="hover:bg-[#fbfdfc]"><td className="p-3"><div className="font-black">{row.participant?.full_name}</div><div className="text-xs text-[var(--muted)]">{row.participant?.participant_no}</div></td><td className="p-3"><StatusBadge status={row.status} /></td><td className="p-3 text-center font-black">{row.claimCount}</td><td className="p-3 text-center font-black">{row.evidenceCount}</td><td className="p-3">{row.assigned ? <span className="text-emerald-700 font-bold"><i className="bi bi-check-circle mr-1" />Sudah</span> : <span className="text-amber-700 font-bold">Belum</span>}</td><td className="p-3 text-xs text-[var(--muted)]">{formatDateTime(row.submitted_at)}</td><td className="p-3 text-right"><Link className="rpl-btn rpl-btn-secondary text-xs" href={`/prodi/peserta/${row.id}`}>Detail <i className="bi bi-chevron-right" /></Link></td></tr>)}</tbody>
          </table>
        </div>
      </section>
    </div>
