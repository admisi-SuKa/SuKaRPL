...
export function ParticipantControls({ applicationId, status, assessors, assignment }: { applicationId: string; status: string; assessors: Assessor[]; assignment?: { assessor1_id: string; assessor2_id: string } | null }) {
  ...
  const canAssign = !["DRAFT","RETURNED","FINAL"].includes(status);
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <section className="rpl-card p-5">
        ...
        <p className="mt-1 text-xs text-[var(--muted)]">Tentukan dua asesor berbeda untuk peserta ini.</p>
